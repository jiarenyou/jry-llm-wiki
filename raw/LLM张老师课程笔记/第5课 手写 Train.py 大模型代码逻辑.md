...
⬅️ 上一课：[[第4课 手写 Model.py 大模型代码逻辑|第4课 Model.py]] | 下一课：[[第6课 手写 Inference.py 大模型代码逻辑|第6课 Inference.py]] ➡️
...
- **参数配置**: 讲解了将学习率 (`learning_rate`)、[[warmup_ratio (预热比例)]]、批次大小 (`batch_size`) 等超参数预先定义的好处，并提到未来会将它们统一放入配置文件中，便于管理。
